    ...
    while running:
        if not pause:
            if findstatus == True:
                ...
                if status == 'casted':
                    if pyautogui.locateOnScreen('polecasted.png', region=(798,339,303,496), confidence = 0.8) == None:
                ...
                if status == 'readytocast' and (waterType == 0 or hasBait == False):
                    ...
                    if status == 'readytocast' and pyautogui.locateOnScreen('f3.png', confidence = 0.8) == None :
                        print('didnt find f3, hook landed - 1st block')
                        status = 'waitingforbite'
                    else:
                        print('hook missed - 1st block')
                        status = 'readytocast'
                ...
                if status == 'readytocast' and waterType > 0 and baitEquipped == True:
                    ...
                    if status == 'readytocast' and pyautogui.locateOnScreen('f3.png', confidence = 0.8) == None :
                        print('didnt find f3, hook landed - 2nd block')
                        status = 'waitingforbite'
                    else:
                        print('hook missed - 2nd block')
                        status = 'readytocast'
                ...
                #Step 5
                if status == 'waitingforbite' and pyautogui.locateOnScreen('polecasted.png', region=(798,339,303,496), confidence = 0.8) != None :
                    status = 'casted'
                    print('rod casted')
            ...
            if status == 'tugtime':
                if pyautogui.locateOnScreen('hold1.png', region=(798,339,303,496), confidence=0.7) != None or pyautogui.locateOnScreen('hold2.png', region=(798,339,303,496), confidence=0.8) != None or pyautogui.locateOnScreen('hold3.png', region=(798,339,303,496), confidence=0.7) != None:
                    pydirectinput.keyDown('-')
                    print('Hold1')
